...
const RiskTable = ({ data, selected, dotColor, skorColor }) => {
  return (
            ...
            {data.map((p) => (
              <tr
                ...
                className={`border-b border-gray-50 cursor-pointer transition-colors ${
                  selected?.daerah === p.daerah ? 'bg-blue-50' : 'hover:bg-gray-50'
                }`}
              >
                <td className="px-4 py-2.5 text-sm flex items-center gap-2">
                  <span 
                    className="w-2 h-2 rounded-full" 
                    style={{ backgroundColor: dotColor(p.index_resiko) }} 
                  />
                  {p.daerah} 
                </td>
                <td 
                  className="px-4 py-2.5 text-sm font-bold text-right"
                  style={{ color: skorColor(p.index_resiko) }}
                >
                  {Number(p.index_resiko).toFixed(1)}
                </td>
                <td className="px-4 py-2.5 text-sm text-gray-600 text-right">
                  {(p.count_danger || 0).toLocaleString()} 
                </td>
              </tr>
            ))}
          </tbody>
